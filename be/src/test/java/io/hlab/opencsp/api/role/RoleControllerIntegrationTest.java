...
class RoleControllerIntegrationTest {
    ...
    @MockBean
    private io.hlab.opencsp.infrastructure.security.JwtUtils jwtUtils;

    @MockBean
    private io.hlab.opencsp.infrastructure.iam.IamClient iamClient;
    ...
    void removeRole_withAdminRole_success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/api/roles/{email}/{role}", TEST_EMAIL, "USER_A")
                        .with(jwt().jwt(jwtWithAdminRole)
                                .authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
    ...
    void getMyRoles_emailNotFound_errorResponse() throws Exception {
        ...
        Jwt jwtWithoutEmail = Jwt.withTokenValue("test-token")
                .header("alg", "RS256")
                .claim("sub", "test-user-id")
                .issuedAt(Instant.now())
                .expiresAt(Instant.now().plusSeconds(3600))
